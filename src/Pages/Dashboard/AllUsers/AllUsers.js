...
const AllUsers = () => {
  ...
  <table className="table w-full">
    ...
    <tbody>
      {
        allUsers?.length === 0 ? <SmallLoading></SmallLoading> : <>
        {
          allUsers?.map((allUser, idx)=><tr>
            <th>
              <label>
                {idx}
              </label>
            ...
            <td><strong>{allUser.role}</strong></td>
            <th>
              <button onClick={()=>handlerDeleteBtn(allUser._id)} className="btn bg-red-500 mr-2">delete</button>
            </th>
          </tr>)
        }
        </>
